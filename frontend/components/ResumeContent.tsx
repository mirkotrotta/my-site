...
const HTMLContent = ({ content }: { content: string }) => {
  if (typeof content !== 'string') {
    console.warn("HTMLContent received non-string content:", content);
    return null; // Or return a default placeholder
  }
  try {
    return <div dangerouslySetInnerHTML={{ __html: content }} />;
  } catch (error) {
    console.error("Error rendering HTMLContent:", error, "Content was:", content);
    return <div className="text-red-500">Error displaying content</div>; // Fallback UI
  }
};
...
export default function ResumeContent({ language: initialLanguage }: ResumeContentProps) {
  ...
  useEffect(() => {
    ...
    fetch(apiUrl)
      .then((res) => {
        if (!res.ok) throw new Error(`API fetch failed: ${res.statusText} (status: ${res.status}, url: ${apiUrl})`);
        return res.json();
      })
      .then((data: ResumeData) => {
        // Basic validation of the received data structure
        if (!data || !data.profile || !data.professional_experience) {
          console.error("Received incomplete resume data structure from API:", data, "URL:", apiUrl);
          throw new Error("Incomplete resume data received from API.");
        }
        setResume(data);
      })
      .catch((err) => {
        console.error(`Error fetching resume data from ${apiUrl}:`, err.message, err);
        setError(t('resume.errors.fetchFailed') + `: ${err.message}`);
        setResume(null);
      })
  ...
  useEffect(() => {
    ...
    fetch("/api/github/")
      .then((res) => {
        if (!res.ok) throw new Error(t('resume.errors.projectsFetchFailed') + ` (status: ${res.status}, url: /api/github/)`);
        return res.json();
      })
      .then((data: Project[]) => setGithubProjects(data || [])) // Ensure githubProjects is always an array
      .catch((err) => {
        console.warn("GitHub API fetch failed (/api/github/):", err.message, err);
  ...
  try {
        ...
        <div className="my-16 grid grid-cols-1 md:grid-cols-8 gap-16 max-w-full mx-auto">
          ...
          <div className="md:col-span-2 flex md:block justify-center">
            <div className="md:sticky md:top-16 w-full flex flex-col justify-center md:justify-start">
              ...
              {resume.profile ? <ProfileCard profile={resume.profile} /> : <p>Profile data missing</p>}
                ...
                {resume.professional_experience?.map((entry, idx) => (
                  <div key={idx} className="border-l border-gray-400 dark:border-gray-600 pl-6 relative">
                    ...
                    <h3 className="text-xl font-normal text-blue-600 dark:text-blue-400 leading-snug">
                      {entry.title || 'N/A'} @ {entry.company || 'N/A'}
                    </h3>
                    <p className="text-sm text-gray-500 mt-1">
                      {entry.start_date || 'N/A'} - {entry.end_date === 'Present' ? t('resume.present') : (entry.end_date || 'N/A')}
                    </p>
                    ...
                    <p className="text-sm text-gray-500 italic mt-1">{entry.location || 'N/A'}</p>
                  </div>
                ))}
              </div>
            </section>
              ...
              <ul className="space-y-4">
                {resume.education?.map((entry, idx) => (
                  <li key={idx} className="mb-2">
                    <h3 className="text-lg font-normal">{entry.institution || 'N/A'}</h3>
                    <p className="text-md text-gray-700 dark:text-gray-200">
                      {entry.degree || 'N/A'} ({entry.year || 'N/A'})
                    </p>
            ...
            <section>
              ...
              <ul className="space-y-4">
                {resume.certifications?.map((cert, idx) => (
                  <li key={idx} className="mb-2">
                    <h3 className="text-lg font-normal">{(cert.name || 'N/A')} {cert.status && <span className="font-normal">({cert.status})</span>}</h3>
            ...
            <section>
              ...
              <ul className="list-disc pl-5 text-lg text-gray-700 dark:text-gray-200">
                {resume.languages?.map((lang, idx) => (
                  <li key={idx}>
                    <HTMLContent content={`<strong>${lang.language || 'N/A'}</strong>: ${lang.level || 'N/A'}`} />
                  </li>
                ))}
              </ul>
            </section>
                ...
                {resume.projects?.map((project, idx) => {
                  ...
                  return (
                    <div key={idx} className="border p-6 hover:shadow-md transition-shadow bg-white dark:bg-gray-800">
                      ...
                      <p className="text-sm text-gray-500 mb-3">{project.stack || 'N/A'}</p>
    ...
    console.error("Critical rendering error in ResumeContent:", renderError, "Resume data was:", resume);
    // It's often good to have a specific state for critical render errors
    // to display a more user-friendly message or a global error boundary.
    setError(t('resume.errors.renderFailed') + `: ${renderError.message}`); 
    // Returning error here might re-trigger render and loop if setError causes re-render
    // A more robust solution might involve a dedicated error display component
    // or allowing the global error boundary of Next.js to catch this.
    // For now, let's ensure the error state is set and rely on the error display logic at the top.
    // To prevent render loops, we might return a static error message directly:
    return <p className="text-lg text-red-700 p-8">A critical error occurred while displaying the resume. Our team has been notified.</p>;
  }
} 